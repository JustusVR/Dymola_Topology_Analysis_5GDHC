...
public
  ...
  Dual_Pipe.Dual_Pipe_HeatLoss_dynamics
                      dual_Pipe(
    length=100,
    m_flow_nominal=1,
    redeclare package Medium = Medium,
    dynamics=false)                    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-2,16})));
  Building.Automated_Model_Building.Building_TA_LookUp Office2(redeclare
      package Medium = Medium, dynamics=false) annotation (Placement(
        transformation(
        extent={{-20,-20},{20,20}},
        rotation=0,
        origin={0,74})));
...
equation
  ...
  connect(Office2.port_a,Office2_node_a. ports[1])
    annotation(Line(points={{-5.71429,54},{-5.71429,48},{-7.6,48},{-7.6,34}},
                                                     color={0, 127, 255}));
  connect(Office2.port_b,Office2_node_b. ports[1])
    annotation(Line(points={{5.71429,54},{5.71429,48},{4,48},{4,44},{2.4,44},{
