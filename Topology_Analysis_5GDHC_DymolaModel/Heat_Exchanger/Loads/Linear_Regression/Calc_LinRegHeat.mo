...
model Calc_LinRegHeat
  ...
  Modelica.Blocks.Interfaces.RealOutput heatEleHP
    annotation (Placement(transformation(extent={{140,-100},{182,-58}})));
  Modelica.Blocks.Interfaces.RealOutput heatTOutlet
...
equation
  ...
  power.u =
    Topology_Analysis_5GDHC.Heat_Exchanger.Loads.Linear_Regression.ext_func_heat_Q(
    {1,dayIntCorrected.y,hourInt.y,monthInt.y,mFlow,tInletC.y,humOut,tOutsideC.y});
  heatEleHP =
    Topology_Analysis_5GDHC.Heat_Exchanger.Loads.Linear_Regression.ext_func_heat_P(
    {1,dayIntCorrected.y,hourInt.y,monthInt.y,mFlow,tInletC.y,humOut,tOutsideC.y});
  heatTOutlet =
    Topology_Analysis_5GDHC.Heat_Exchanger.Loads.Linear_Regression.ext_func_heat_T(
    {1,dayIntCorrected.y,hourInt.y,monthInt.y,mFlow,tInletC.y,humOut,tOutsideC.y});
  ...
  connect(month, monthInt.u)
    annotation (Line(points={{-120,-40},{-82,-40}}, color={255,127,0}));
  connect(power.y, heatDemand) annotation (Line(points={{105,80},{132,80},{132,
