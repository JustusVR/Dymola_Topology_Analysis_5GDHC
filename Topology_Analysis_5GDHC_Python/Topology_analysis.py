...
def SetScenarioConnectionMatrices(activeProsumers):
    ...
    for l in range((len(activeProsumers)-1), max(len(ProsumerConnections)+1,3)):
        for subset in itertools.combinations(ProsumerConnections, l):
            ...
            if newNetworkCheck(activeProsumers, selectedConnections) <> NetworkCheck(connectedNodes, activeProsumers):
                print "newNetworkCheck:", newNetworkCheck(activeProsumers, selectedConnections)
                print "oldNetworkCheck:", NetworkCheck(connectedNodes, activeProsumers)
                print "active Prosumers:",
                for ap in activeProsumers:
                    print ap.name,
                print ""
                for sc in selectedConnections:
                    print sc.model1.name, sc.model2.name, ",",
                print ""
...
def newNetworkCheck(activeProsumers, selectedConnections):
    check = True
    scenarioMatrix = transformToConnectionMatrix(selectedConnections)
    for ap in activeProsumers:
        r = [row[modellist.index(ap)] for row in scenarioMatrix]
        if 1 not in r:
            check = False
    return check

def transformToConnectionMatrix(selectedCombinations):
    size = len(modellist)
    a = np.zeros((size, size), dtype=np.int16)
    for sc in selectedCombinations:
        x = modellist.index(sc.model1)
        y = modellist.index(sc.model2)
        a[x][y] = 1
        a[y][x] = 1
    return a
...
def testNewNetworkCheck():
   # global allNodes
    allNodes = [1,2,3,4]
    activeNodes = [1,2,3,4]
    connectedNodes = [[1,2],[2,3],[1,3]]
    newNetworkCheck(allNodes, activeNodes, connectedNodes)
