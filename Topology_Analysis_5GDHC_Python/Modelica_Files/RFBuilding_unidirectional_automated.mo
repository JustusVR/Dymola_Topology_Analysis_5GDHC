...
model RFBuilding_unidirectional_automated
  Connector.Connector pipe_a1 (redeclare final package Medium = Medium)
  ...
  Connector.Connector pipe_a2 (redeclare final package Medium = Medium)
  ...
  Connector.Connector pipe_a3 (redeclare final package Medium = Medium)
    annotation (Placement(transformation(extent={{-260,290},{-240,310}}),
        iconTransformation(extent={{-300,-20},{-260,20}})));
  Modelica.Fluid.Interfaces.FluidPort_a port_a3(redeclare package Medium = Medium)
    annotation (Placement(transformation(extent={{-290,290},{-270,310}})));
...
equation
  connect(pipe_a1.port_b, del_A.ports[3])
    annotation (Line(points={{-230,-290},{-210,290},{-230,-0},{-210,-0}}, color={0,0,127}));
  connect(pipe_a2.port_b, del_A.ports[4])
    annotation (Line(points={{-230,10},{-210,0},{-230,-0},{-210,-0}}, color={0,0,127}));
  connect(pipe_a3.port_b, del_A.ports[5])
    annotation (Line(points={{-230,310},{-210,10},{-230,-0},{-210,-0}}, color={0,0,127}));
  connect(port_a1, pipe_a1.port_a)
    annotation (Line(points={{-260,-290},{-230,-290}}, color={0, 127, 255}));
  connect(port_a2, pipe_a2.port_a)
    annotation (Line(points={{-260,10},{-230,10}}, color={0, 127, 255}));
  connect(port_a3, pipe_a3.port_a)
    annotation (Line(points={{-260,310},{-230,310}}, color={0, 127, 255}));

  connect(del_A.ports[1], pump_hea.port_a) annotation (Line(points={{-210,4},{
          -210,150},{-150,150}},                            color={0,127,255}));
  connect(eleHP, eleHP)
  ...
  connect(pump_Cool.P, add.u2) annotation (Line(points={{-129,-181},{-129,
          -180},{-100,-180},{-100,236},{-72,236}}, color={0,0,127}));
  ...
  connect(cool.port_b, del_B.ports[2]) annotation (Line(points={{24,-191},{
          240,-191},{240,18}}, color={0,127,255}));
  ...
  connect(hea.Q_flow, Qflow_heat) annotation (Line(points={{24.4,162.8},{
          179.2,162.8},{179.2,310},{180,310}}, color={0,0,127}));
  connect(const.y, eleHP) annotation (Line(points={{113,240},{120,240},{120,
          310}}, color={0,0,127}));
  ...
  connect(port_b, del_B.ports[3]) annotation (Line(points={{280,0},{260,0},{260,
          0},{242.667,0},{242.667,18}},      color={0,127,255}));
  annotation (
